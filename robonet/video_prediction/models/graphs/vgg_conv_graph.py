...
def _cast_down(tensor, float16=False):
    if float16:
    ...
    def build_graph(self, mode, inputs, hparams, scope_name='flow_generator'):
        ...
        with tf.variable_scope(self._scope_name) as graph_scope:
            ...
            for t in range(hparams.sequence_length - 1):
                ...
                action_state_vector = _cast_down(inputs['actions'][t], hparams.float16)
                if t < hparams.context_frames:
                    input_image = _cast_down(inputs['images'][t], hparams.float16)
                else:
                    casted_real = _cast_down(inputs['images'][t], hparams.float16)
                    casted_gen = _cast_down(outputs['gen_images'][-1], hparams.float16)
                ...
                for i, op in enumerate(self._enc_ops):
                    ...
                    if isinstance(op, layers.Conv2D):
                        ...
                        encoded_imgs = _cast_down(encoded_imgs, hparams.float16)
                        norm_ctr += 1
                ...
                # decoder attention
                enc_out = _cast_down(enc_out, hparams.float16)
                ...
                decoder_out = _cast_down(dec_lstm_out, hparams.float16)
                for op in self._dec_ops:
                    ...
                    if isinstance(op, layers.Conv2D):
                        ...
                        decoder_out = _cast_down(decoder_out, hparams.float16)
                        norm_ctr += 1
