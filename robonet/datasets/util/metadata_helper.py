    ...
    def select_objects(self, obj_class_name):
        if isinstance(obj_class_name, str):
            return self._meta_data[
                [obj_class_name in x for x in self._meta_data["object_classes"]]
            ]
        return self._meta_data[
            [set(obj_class_name) == set(x) for x in self._meta_data["object_classes"]]
        ]
    ...
    with h5py.File(io.BytesIO(buf), "r") as hf:
        ...
        if n_cams:
            ...
            if hf["env"].attrs["cam_encoding"] == "mp4":
                meta_data_dict["frame_dim"] = hf["env"]["cam0_video"]["frames"].attrs[
                    "shape"
                ][:2]
                meta_data_dict["img_T"] = hf["env"]["cam0_video"]["frames"].attrs["T"]
                meta_data_dict["img_encoding"] = "mp4"
                meta_data_dict["image_format"] = hf["env"]["cam0_video"][
                    "frames"
                ].attrs["image_format"]
            else:
                meta_data_dict["frame_dim"] = hf["env"]["cam0_video"]["frame0"].attrs[
                    "shape"
                ][:2]
                meta_data_dict["image_format"] = hf["env"]["cam0_video"][
                    "frame0"
...
def get_metadata_frame(files):
    if isinstance(files, str):
        ...
        if os.path.exists("{}/meta_data.pkl".format(base_path)):
            meta_data = pd.read_pickle(
                "{}/meta_data.pkl".format(base_path), compression="gzip"
            )
