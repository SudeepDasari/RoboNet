...
def load_states(file_pointer, meta_data, hparams):
    ...
    if hparams.target_sdim == sdim:
        ...
        sdim < hparams.target_sdim and hparams.state_mismatch & STATE_MISMATCH.PAD_ZERO
    ):
        ...
        raise ValueError(
            "file sdim - {}, target sdim - {}, pad behavior - {}".format(
                sdim, hparams.target_sdim, hparams.state_mismatch
            )
        )
...
def load_actions(file_pointer, meta_data, hparams):
    ...
    if hparams.target_adim == adim:
        ...
        hparams.target_adim == adim + 1
        and hparams.impute_autograsp_action
        ...
        adim < hparams.target_adim
        and hparams.action_mismatch & ACTION_MISMATCH.PAD_ZERO
    ):
        pad = np.zeros((a_T, hparams.target_adim - adim), dtype=np.float32)
        return np.concatenate((file_pointer["policy"]["actions"][:], pad), axis=-1)

    elif (
        adim > hparams.target_adim and hparams.action_mismatch & ACTION_MISMATCH.CLEAVE
    ):
        ...
        raise ValueError(
            "file adim - {}, target adim - {}, pad behavior - {}".format(
                adim, hparams.target_adim, hparams.action_mismatch
            )
        )
...
def load_data(f_name, file_metadata, hparams, rng=None):
    ...
    with h5py.File(io.BytesIO(buf), "r") as hf:
        ...
        actions = load_actions(hf, file_metadata, hparams).astype(np.float32)[
            start_time : start_time + n_states - 1
        ]
        states = load_states(hf, file_metadata, hparams).astype(np.float32)[
